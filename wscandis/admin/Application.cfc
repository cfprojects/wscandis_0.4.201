<cfcomponent displayname="Controller">
	...
	<cfset this.topLevelErrorRecipient = "yourname@yourdomain.com">
	<cfset this.topLevelErrorSender = "yourname@yourdomain.com">
	<cfset this.restartKey = "restart_dec_18">
	...
	<cffunction name="onRequestStart">
		...
		<cfparam name="resetApp" default="false"> <!--- use to reset the application state --->
 		...
 		<cfscript>
			...
			// returns true if everything went ok and we can process the event
			// if returns false, then something went wrong and no event will be processed
			bRet = appEventHandler.onRequestStart();
			
			// execute event only if the onRequestStart execution was ok, 
			// and the event string has the correct form
			if(bRet and listLen(event,".") eq 2) {
				// Parse event handler
